...
class SpellCheckerProvider {
    ...
    doSpellCheck(textDocument) {
        ...
        for (let i in tokens) {
            if (DEBUG) {
                ...
                let seconds = Math.floor((currTime - startTime) / 1000);
    ...
    provideCodeActions(document, range, context, token) {
        ...
        if (!diagnostic)
            return null;
        // Get word
        let match = diagnostic.message.match(/^Spelling \[\ (.+)\ \]\:/);
        if (DEBUG && match) {
    ...
    fixSuggestionCodeAction(document, diagnostic, word, suggestion) {
        let docWord = document.getText(diagnostic.range);
        if (word == docWord) {
    ...
    ignoreCodeAction(document, word) {
    ...
    alwaysIgnoreCodeAction(document, word) {
        if (DEBUG) {
            console.log(word);
    ...
    addWordToIgnoreList(word, save) {
        ...
        if (this.settings.ignoreWordsList.indexOf(word) < 0) {
            ...
            if (save) {
                let userSettingsData = vscode.workspace.getConfiguration('spellchecker');
    ...
    addWordToAlwaysIgnoreList(word) {
    ...
    migrateWorkspaceSettings() {
        ...
        vscode.window.showQuickPick(options, qpOptions).then(val => {
            ...
            let settings = JSON.parse(jsonMinify(fs.readFileSync(SpellCheckerProvider.CONFIGFILE, 'utf-8')));
            if (migrate) {
                // Check user settings
                let userSettingsData = vscode.workspace.getConfiguration('spellchecker');
