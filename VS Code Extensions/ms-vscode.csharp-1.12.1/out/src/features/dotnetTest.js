...
class TestManager extends abstractProvider_1.default {
    ...
    _runTest(fileName, testMethod, testFrameworkName) {
        const request = {
            FileName: fileName,
    ...
    _getLaunchConfigurationForVSTest(fileName, testMethod, testFrameworkName, debugEventListener) {
        const output = this._getOutputChannel();
    ...
    _getLaunchConfigurationForLegacy(fileName, testMethod, testFrameworkName) {
        const output = this._getOutputChannel();
        // Listen for test messages while getting start info.
        const listener = this._server.onTestMessage(e => {
            output.appendLine(e.Message);
    ...
    _getLaunchConfiguration(debugType, fileName, testMethod, testFrameworkName, debugEventListener) {
        switch (debugType) {
            case 'legacy':
