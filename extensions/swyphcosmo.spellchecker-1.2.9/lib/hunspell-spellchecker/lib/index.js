...
var Spellchecker = function(dictionary, options) {
    this.dict = null;
...
Spellchecker.prototype.suggest = function (word, limit) {
    ...
    */
            ...
            for (var i = 0, _len = word.length + 1; i < _len; i++) {
                ...
                if (s[1]) {
                    for (var j = 0, _jlen = self.dict.alphabet.length; j < _jlen; j++) {
                        ...
                        if( self.dict.alphabet[j] != s[1].substring(0,1) ){
                            rv.push(s[0] + self.dict.alphabet[j] + s[1].substring(1));
                        }
                    }
                }
                ...
                if (s[1]) {
                    for (var j = 0, _jlen = self.dict.alphabet.length; j < _jlen; j++) {
                        rv.push(s[0] + self.dict.alphabet[j] + s[1]);
                    }
                }
            }
        }
    ...
    function correct(word, options) {
        // Get the edit-distance-1 and edit-distance-2 forms of this word.
        var ed1 = edits1([word]);
        var corrections = known(ed1);

        if (!options.noDistance2) {
            var ed2 = edits1(ed1);
    ...
    return correct(word, this.options);
};
