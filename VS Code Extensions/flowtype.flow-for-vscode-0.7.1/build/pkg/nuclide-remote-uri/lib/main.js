...
var _path6 = _interopRequireDefault(_path5);
...
function parse(uri) {
  ...
  var parsedUri = _url2.default.parse(_escapeBackslashes(uri));

  (0, _assert2.default)(parsedUri.path, 'Nuclide URIs must contain paths, \'' + String(parsedUri.path) + '\' found while parsing \'' + uri + '\'');
  ...
  (0, _assert2.default)(parsedUri.pathname, 'Nuclide URIs must contain pathnamess, \'' + String(parsedUri.pathname) + '\' found while parsing \'' + uri + '\'');
...
function parseRemoteUri(remoteUri) {
  ...
  (0, _assert2.default)(parsedUri.hostname, 'Remote Nuclide URIs must contain hostnames, \'' + String(parsedUri.hostname) + '\' found ' + ('while parsing \'' + remoteUri + '\''));
...
function join(uri) {
  ...
  if (isRemote(uri)) {
    var _parseRemoteUri = parseRemoteUri(uri),
        _hostname = _parseRemoteUri.hostname,
        _path = _parseRemoteUri.path;
...
function normalize(uri) {
  ...
  if (isRemote(uri)) {
    var _parseRemoteUri2 = parseRemoteUri(uri),
        _hostname2 = _parseRemoteUri2.hostname,
        _path2 = _parseRemoteUri2.path;

    return createRemoteUri(_hostname2, uriPathModule.normalize(_path2));
  } else {
...
function dirname(uri) {
  ...
  if (isRemote(uri)) {
    var _parseRemoteUri3 = parseRemoteUri(uri),
        _hostname3 = _parseRemoteUri3.hostname,
        _path3 = _parseRemoteUri3.path;

    return createRemoteUri(_hostname3, uriPathModule.dirname(_path3));
  } else {
...
function uriToNuclideUri(uri) {
  var urlParts = _url2.default.parse(_escapeBackslashes(uri), false);
...
function nuclideUriToDisplayString(uri) {
  if (isRemote(uri)) {
    ...
    try {
      for (var _iterator = hostFormatters[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function resolve(uri) {
  ...
  if (isRemote(uri)) {
    var _parseRemoteUri4 = parseRemoteUri(uri),
        _hostname5 = _parseRemoteUri4.hostname,
        _path4 = _parseRemoteUri4.path;

    paths.splice(0, 0, _path4);
...
function expandHomeDir(uri) {
  ...
  return _path6.default.posix.resolve(HOME, uri.replace('~', '.'));
}
...
function splitPathList(paths) {
  (0, _assert2.default)(paths.indexOf(REMOTE_PATH_URI_PREFIX) < 0, 'Splitting remote URIs is not supported');
...
function joinPathList(paths) {
  ...
  (0, _assert2.default)(paths.every(function (path) {
...
function ensureLocalPrefix(uri) {
  ...
  (0, _assert2.default)(!isRemote(uri), 'Local prefix can not be added to a remote path');
  (0, _assert2.default)(!isAbsolute(uri), 'Local prefix can not be added to an absolute path');
...
function _pathModuleFor(uri) {
  var posixPath = _path6.default.posix;
  var win32Path = _path6.default.win32;
